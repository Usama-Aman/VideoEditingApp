        ...
        BackgroundExecutor.execute(object : BackgroundExecutor.Task("", 0L, "") {
            override fun execute() {
                try {
                    ...
                    val frameWidth = ((initialBitmap?.width?.toFloat()
                        ?.div(initialBitmap.height.toFloat()))?.times(frameHeight.toFloat()))?.toInt()
...
//                                bitmap = Bitmap.createBitmap(bitmap, frameWidth, frameHeight, cropWidth, bitmap.height)
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
